...
def test_download_page_with_some_unavailable_resources(requests_mock, tmpdir):
    ...
    requests_mock.get(page_url + "/unavailable.png", status_code=HTTPStatus.NOT_FOUND)
...
def test_download_page_with_resources(requests_mock, tmpdir):
    page_html = tests.utils.read(os.path.join(FIXTURES_PATH, "page.html"))
    application_css_content = tests.utils.read(os.path.join(FIXTURES_PATH, "assets/application.css"))
    application_js_content = tests.utils.read(os.path.join(FIXTURES_PATH, "assets/application.js"))
    runtime_js_content = tests.utils.read(os.path.join(FIXTURES_PATH, "assets/runtime.js"))
    python_png_binary = tests.utils.read(os.path.join(FIXTURES_PATH, "assets/python.png"), mode="rb")
    ...
    assert tests.utils.read(page_file_path) == tests.utils.read(os.path.join(FIXTURES_PATH, "expected-page.html"))
